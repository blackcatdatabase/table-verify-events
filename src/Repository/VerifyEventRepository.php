...
use BlackCat\Database\Packages\VerifyEvents\Dto\VerifyEventDto as Dto;
use BlackCat\Database\Packages\VerifyEvents\Mapper\VerifyEventDtoMapper as RowMapper;
...
final class VerifyEventRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\VerifyEvents\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\VerifyEvents\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\VerifyEvents\Dto\VerifyEventDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\VerifyEvents\Dto\VerifyEventDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\VerifyEvents\Mapper\VerifyEventDtoMapper::fromRow($row);
    }

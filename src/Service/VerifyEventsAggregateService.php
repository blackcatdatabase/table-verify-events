 ...
 */
final class VerifyEventsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private VerifyEventRepository $verifyEventRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'verify_events' => $this->verifyEventRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\VerifyEvents\ModuleException("Record {$idStr} not found for locking.");
            }
